...
CServerDownloadThread * CServerDownloadThread::CreateCServerDownloadThread(CString stFilePath, SOCKET soc, CString csIPnPort, FTPMode eMode)
{
...
int CServerDownloadThread::Run()
{
	...
	if (m_eMode == FTPMode::Active)
	{
		...
		soc = accept(m_Socket,(sockaddr*)&sockaddrClient, &iSize);
	...
	else if (m_eMode == FTPMode::Passive)
	{
		...
		addrServer.sin_addr.s_addr = inet_addr(csIP.GetBuffer());

		if (connect(soc, (sockaddr*)&addrServer, sizeof(addrServer)) == SOCKET_ERROR)
		{
			m_File.Close();
